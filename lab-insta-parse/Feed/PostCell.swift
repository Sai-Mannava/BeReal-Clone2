    ...
    func configure(with post: Post) {
        ...
        var dataLabelText = ""
           ...
           if let date = post.createdAt {
               dataLabelText += DateFormatter.postFormatter.string(from: date)
           } else {
               dataLabelText += "Unknown Date"
           }
           
           // Configure the location display
           if let location = post.location {
               // Assuming location is stored as a string. You may need to format it if it's stored differently (e.g., as coordinates).
               dataLabelText += " | \(location)"
           } else {
               dataLabelText += " | Davis, CA"
           }
           
           // Set the combined text to the dataLabel
           dateLabel.text = dataLabelText
